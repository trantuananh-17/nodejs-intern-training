...
async function getBooks(ctx: Context) {
  try {
    ...
    const res = await bookRepository.getBooks(Number(page), Number(limit), sort as 'asc' | 'desc');
...
async function getBookById(ctx: Context) {
  try {
...
async function createBook(ctx: Context) {
  try {
...
async function updateBook(ctx: Context) {
  try {
    const bookId = Number(ctx.params.bookId);
    const bookData = ctx.request.body;
...
async function deleteBook(ctx: Context) {
  try {
    const bookId = Number(ctx.params.bookId);
...
async function renderProductsPage(ctx: Context) {
  const products = await bookRepository.getBooks(1, 15, 'desc');

  await ctx.render('pages/products', { products: products.data });
}

async function renderProductInfoPage(ctx: Context) {
  const bookId = Number(ctx.params.bookId);
